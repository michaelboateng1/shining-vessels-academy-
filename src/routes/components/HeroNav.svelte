<script>
	import HeroNavCard from './HeroNavCard.svelte';
	import ShapeDesign from './shapeDesign.svelte';

	const cardList = [
		{
			title: 'Academics',
			icon: 'fa-solid fa-school sm:pt-10 pt-5 text-5xl sm:text-6xl text-[#04095d]'
		},
		{
			title: 'Academics',
			icon: 'fa-solid fa-brain sm:pt-10 pt-5 text-5xl sm:text-6xl text-[#04095d]'
		},
		{
			title: 'Academics',
			icon: 'fa-solid fa-book sm:pt-10 pt-5 text-5xl sm:text-6xl text-[#04095d]'
		}
	];
</script>

<div class=" mt-20 flex w-full flex-wrap items-center justify-center gap-10 sm:justify-evenly">
	{#each cardList as card, i}
		<HeroNavCard {i}>
			<i class={card.icon}></i>
			<svg
				width="100px"
				height="50px"
				viewBox="0 0 512 512"
				xmlns="http://www.w3.org/2000/svg"
				xmlns:xlink="http://www.w3.org/1999/xlink"
				aria-hidden="true"
				role="img"
				class="iconify iconify--fxemoji"
				preserveAspectRatio="xMidYMid meet"
				fill="#fb8961"
				transform="rotate(180)"
				><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g
					id="SVGRepo_tracerCarrier"
					stroke-linecap="round"
					stroke-linejoin="round"
				></g><g id="SVGRepo_iconCarrier"
					><path
						fill="#fb8961"
						d="M478.679 226.691c-31.649 0-60.946 19.093-80.381 52.383l-.004.006c-8.907 15.252-20.189 23.999-30.954 23.999c-10.769 0-22.052-8.75-30.958-24.006l-.014-.023c-19.435-33.276-48.726-52.36-80.368-52.36c-31.649 0-60.947 19.093-80.381 52.382c-8.907 15.257-20.19 24.007-30.958 24.007c-10.768 0-22.052-8.75-30.958-24.007c-19.435-33.289-48.732-52.382-80.381-52.382c-15.803 0-28.614 12.811-28.614 28.614s12.811 28.614 28.614 28.614c10.768 0 22.052 8.75 30.958 24.007c19.435 33.289 48.732 52.382 80.381 52.382c31.648 0 60.946-19.093 80.381-52.382c8.907-15.257 20.19-24.007 30.958-24.007c10.766 0 22.048 8.747 30.954 23.999l.004.006c19.435 33.29 48.732 52.383 80.382 52.383c31.644 0 60.939-19.088 80.373-52.369l.008-.013c8.906-15.256 20.189-24.006 30.957-24.006c15.803 0 28.614-12.812 28.614-28.614c.001-15.802-12.811-28.613-28.613-28.613z"
					></path></g
				></svg
			>
			<p class="pb-5 text-xl font-semibold text-[#04095d] sm:pb-10 sm:text-2xl">{card.title}</p>
		</HeroNavCard>
	{/each}
</div>
<ShapeDesign />
